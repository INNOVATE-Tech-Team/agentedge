...
// Flips any 'active' roster row with >=3 effective deals to 'graduated'.
// Cheap enough to run on every page load — only touches rows that have a
// deals_override, a linked Darwin agent, or at least one logged deal.
...
function launch_roster_resolve_agent(array $directory, string $name, string $state): array {
    ...
    $crm   = $storedEmail === '' && $staff === null ? lr_lookup_by_name($directory['crmByName'], $directory['crmByFirstLast'], $nameLower) : null;
