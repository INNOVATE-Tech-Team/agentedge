...
function user_can_see_folder(array $folder, bool $admin, string $email): bool {
    ...
    if ($folder['visibility'] === 'personal') {
        ...
        return ($folder['owner_email'] ?? '') === $email;
...
// Can the current user write (upload/delete) into this specific folder?

// Admins always can; a non-admin can only in their own personal folder.

$canWrite = $admin || ($folder['visibility'] === 'personal' && ($folder['owner_email'] ?? '') === $email);
...
echo json_encode(['folders' => $subFolders, 'files' => $files, 'can_write' => $canWrite]);
