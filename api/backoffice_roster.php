...
$mcOptsAssign = local_db()
    ->query("SELECT slug, name FROM market_centers WHERE enabled=1 ORDER BY sort_ord, name")
    ->fetchAll(PDO::FETCH_ASSOC);
...
// MC metadata map: display name → {bic_email, mc_leader_email} for heading chips
$mcMeta = [];
foreach (local_db()->query("SELECT name, bic_email, mc_leader_email FROM market_centers")->fetchAll(PDO::FETCH_ASSOC) as $row) {
    $mcMeta[$row['name']] = $row;
}
?>
<!DOCTYPE html>
...
/* Rename MC */
.btn-rename{background:none;border:none;cursor:pointer;padding:2px 5px;opacity:.4;
            font-size:13px;line-height:1;border-radius:3px;transition:opacity .15s}
.btn-rename:hover{opacity:1;background:#e8f5e9}
.mc-rename-wrap{display:none;align-items:center;gap:6px;flex:1}
.mc-rename-wrap.open{display:flex}
.mc-rename-input{font-size:12px;padding:3px 8px;border:1px solid var(--green);border-radius:4px;
                 background:#fff;flex:1;min-width:0;max-width:260px}
.btn-rename-save{padding:3px 10px;background:var(--green);color:#111;border:0;border-radius:4px;
                 font-size:11px;font-weight:700;cursor:pointer;white-space:nowrap}
.btn-rename-cancel{padding:3px 8px;border:1px solid #ccc;background:#fff;color:#555;
                   border-radius:4px;font-size:11px;cursor:pointer}
...
<div class="content">
  ...
  <div class="wrap">
    ...
    <div class="detail-panel">
      ...
      <?php $mcJson = htmlspecialchars(json_encode($mc), ENT_QUOTES); ?>
      <div>
        <div class="mc-heading">
          <input type="checkbox" class="mc-sel-all" title="Select all in this MC"
                 onchange="toggleMcAll(this, <?= $mcJson ?>)">
          <span class="mc-name-label"><?= htmlspecialchars($mc) ?> — <?= count($agents) ?> agent<?= count($agents)!==1?'s':'' ?></span>
          <?php if (!empty($mcMeta[$mc]['bic_email'])): ?>
          <span class="mc-role-chip mc-bic-chip">BIC: <?= htmlspecialchars($mcMeta[$mc]['bic_email']) ?></span>
          <?php endif; ?>
          <?php if (!empty($mcMeta[$mc]['mc_leader_email'])): ?>
          <span class="mc-role-chip mc-leader-chip">Leader: <?= htmlspecialchars($mcMeta[$mc]['mc_leader_email']) ?></span>
          <?php endif; ?>
          <?php if (is_super_admin()): ?>
          <button class="btn-rename" title="Rename this Market Center"
                  onclick="openRename(this, <?= $mcJson ?>)">✏️</button>
          <div class="mc-rename-wrap">
            <input class="mc-rename-input" type="text" value="<?= htmlspecialchars($mc) ?>" maxlength="80"
                   onkeydown="if(event.key==='Enter')saveRename(this.closest('.mc-heading'),<?= $mcJson ?>);if(event.key==='Escape')closeRename(this.closest('.mc-heading'))">
            <button class="btn-rename-save" onclick="saveRename(this.closest('.mc-heading'),<?= $mcJson ?>)">Save</button>
            <button class="btn-rename-cancel" onclick="closeRename(this.closest('.mc-heading'))">Cancel</button>
          </div>
          <?php endif; ?>
        </div>
                  ...
                  <div class="move-mc-inline" id="move-mc-<?= $a['id'] ?>">
                    <select class="move-mc-select">
                      ...
                      <option value="<?= htmlspecialchars($opt['name']) ?>"
                        <?= $opt['name']===$mc?' selected':'' ?>><?= htmlspecialchars($opt['name']) ?></option>
                      <?php endforeach; ?>
                    </select>
...
// ── Rename MC ────────────────────────────────────────────────────────────────
function openRename(btn, mcName) {
    const heading = btn.closest('.mc-heading');
    heading.querySelector('.mc-name-label').style.display = 'none';
    btn.style.display = 'none';
    const wrap = heading.querySelector('.mc-rename-wrap');
    wrap.classList.add('open');
    wrap.querySelector('.mc-rename-input').select();
}

function closeRename(heading) {
    heading.querySelector('.mc-name-label').style.display = '';
    heading.querySelector('.btn-rename').style.display = '';
    heading.querySelector('.mc-rename-wrap').classList.remove('open');
}

function saveRename(heading, oldName) {
    const input    = heading.querySelector('.mc-rename-input');
    const newName  = input.value.trim();
    if (!newName || newName === oldName) { closeRename(heading); return; }
    ...
    .then(r => r.ok ? r.json() : r.text().then(t => { throw new Error('HTTP '+r.status+': '+t.substring(0,300)); }))
    .then(d=>{
        if (!d.ok) { alert('Rename failed: ' + (d.error||'Unknown')); return; }
        // Update the label text and close
        const label = heading.querySelector('.mc-name-label');
        label.textContent = newName + ' — ' + d.count + ' agent' + (d.count!==1?'s':'');
        // Update data-mc on all checkboxes in this MC group
        heading.parentElement.querySelectorAll('.agent-cb,.mc-sel-all')
            .forEach(cb => { if(cb.dataset.mc===oldName) cb.dataset.mc=newName; });
        // Update rename input default value for next edit
        input.value = newName;
        closeRename(heading);
    })
