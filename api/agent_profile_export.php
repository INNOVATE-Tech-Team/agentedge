<?php
// Agent profile export — bio + headshot, for coastline-server's nightly
// agent-sites sync (activates website.innovateonline.com/<slug> pages).
// Token-gated the same way api/roster_export.php and api/onboard_push.php
// are (crm_token) — no login session involved, this is server-to-server.
//
// GET /api/agent_profile_export.php?token=...
//   Bulk metadata only (no image bytes) — cheap to call for the whole roster.
//   Response: { profiles: [{ email, bio, headshot_uploaded_at }] }
//
...
// ── Bulk bio + headshot-freshness metadata ─────────────────────────────
header('Content-Type: application/json');
$rows = $pdo->query(
    "SELECT i.email, i.bio,
