...
$savedId = trim($_GET['saved_id'] ?? '');
if ($savedId === '') { http_response_code(400); echo json_encode(['ok' => false, 'error' => 'saved_id is required']); exit; }

$c     = cfg();
$base  = rtrim($c['crm_base'] ?? 'https://bold360.vip/api', '/');
$token = $c['crm_token'] ?? '';
$qs    = http_build_query(['token' => $token, 'email' => $agent['email']]);
$url   = $base . '/public/agentedge/buyback/hotdeals/saved/' . rawurlencode($savedId) . '/delete?' . $qs;
