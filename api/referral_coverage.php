...
$rows = local_db()->query(
    "SELECT mls_name, agent_label, counties, cities, townships, zips, states, communities, market_centers, specific_agents FROM mls_referral_coverage ORDER BY mls_name"
...
$out = array_map(function($r) {
    return [
        ...
        // Agents (by email, already lowercased on save) manually attached to
        // this row so they surface for its location matches regardless of
        // their own MLS Board or market center -- the escape hatch for
        // someone who personally covers an area their MLS membership doesn't.
        'specificAgents' => split_list($r['specific_agents']),
    ];
}, $rows);
