...
// Cleans a comma/semicolon/newline-separated list of agent emails into a
// deduped, lowercased, comma-joined string -- lowercased because it's
// matched against agent.email in the Agent Roster's referral filter, which
// lowercases everything it compares.
function clean_specific_agents(string $s): string {
    $out = [];
    foreach (preg_split('/[,;\n\r]+/', $s) as $p) {
        $p = strtolower(trim($p));
...
function row_payload(array $r): array {
    return [
        ...
        'specific_agents'=> $r['specific_agents'],
        'notes'          => $r['notes'],
    ];
}
...
switch ($action) {
    ...
    case 'add':
        ...
        try {
            $s = $db->prepare(
                "INSERT INTO mls_referral_coverage (mls_name,agent_label,counties,cities,townships,zips,states,communities,market_centers,specific_agents,notes)
                 VALUES (?,?,?,?,?,?,?,?,?,?,?)"
            );
            $s->execute([
                $name,
                ...
                clean_specific_agents($body['specific_agents'] ?? ''),
                trim($body['notes']     ?? ''),
            ]);
        ...
        try {
            $s = $db->prepare(
                "UPDATE mls_referral_coverage
                 SET mls_name=?, agent_label=?, counties=?, cities=?, townships=?, zips=?, states=?, communities=?, market_centers=?, specific_agents=?, notes=?, updated_at=datetime('now')
                 WHERE id=?"
            );
            $s->execute([
                $name,
