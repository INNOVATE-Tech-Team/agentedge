...
// passcode, dial-in numbers, SIP info) whenever a training event is scheduled as
// a Zoom meeting — the join link already shows via the location line, so drop
// this boilerplate rather than showing the raw dial-in text under every event.
...
$myStatus = [];
if ($email) {
    $stmt = local_db()->prepare("SELECT event_id, status FROM training_rsvps WHERE agent_email=?");
    $stmt->execute([$email]);
    foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $r) $myStatus[$r['event_id']] = $r['status'];
}

$regCounts = [];
foreach (local_db()->query("SELECT event_id, COUNT(*) AS cnt FROM training_rsvps WHERE status='registered' GROUP BY event_id")->fetchAll(PDO::FETCH_ASSOC) as $r) {
...
$regDescs   = [];
$regSlugs   = [];
foreach (local_db()->query("SELECT event_id, capacity, reg_description, reg_slug FROM training_events")->fetchAll(PDO::FETCH_ASSOC) as $r) {
