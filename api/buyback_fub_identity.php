...
$fubUserId = isset($body['fub_user_id']) ? (int)$body['fub_user_id'] : 0;
if ($fubUserId <= 0) { http_response_code(400); echo json_encode(['ok' => false, 'error' => 'fub_user_id is required']); exit; }

$c     = cfg();
$base  = rtrim($c['crm_base'] ?? 'https://bold360.vip/api', '/');
$token = $c['crm_token'] ?? '';
$url   = $base . '/public/agentedge/buyback/fub-identity' . ($token ? '?token=' . urlencode($token) : '');
