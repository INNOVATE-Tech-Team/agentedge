...
// POST {action:'set_status'}      → active | confirmed | on_hold | graduated | dropped
...
if ($action === 'update_fields') {
    ...
    $fields = [];
    $params = [];
    foreach (['agent_name', 'agent_email', 'agent_phone', 'state', 'office', 'coach', 'start_date', 'notes'] as $col) {
        if (!array_key_exists($col, $body)) continue;
        $val = trim((string)$body[$col]);
        if ($col === 'agent_email') $val = strtolower($val);
        $fields[] = "$col=?";
        $params[] = $val;
    }
...
if ($action === 'set_status') {
    ...
    if (!$id || !in_array($status, ['active', 'confirmed', 'on_hold', 'graduated', 'dropped'], true)) jerr('id and a valid status required');
