...
// Every agent gets exactly one personal folder ("My Documents"), auto-provisioned

// on first visit. Visibility is scoped to the owner only — see

// vault_visible_root_folders(), api/vault_browse.php, api/vault_download.php.

if ($email) {

    $pf = $db->prepare("SELECT id FROM vault_folders WHERE visibility='personal' AND owner_email=?");

    $pf->execute([$email]);

    $personalFolderId = $pf->fetchColumn();

    if (!$personalFolderId) {

        $personalFolderId = 'vf-personal-' . substr(md5($email), 0, 16);

        $db->prepare("INSERT OR IGNORE INTO vault_folders

                      (id,parent_id,name,visibility,dept_slug,owner_email,created_by,created_at)

                      VALUES (?,NULL,'My Documents','personal','',?,?,datetime('now'))")

           ->execute([$personalFolderId, $email, $email]);
...
function vault_visible_root_folders(bool $admin, array $userDeptSlugs, string $email): array {
    ...
    return array_values(array_filter($all, function($f) use ($admin, $userDeptSlugs, $email) {

        if ($f['visibility'] === 'public')   return true;

        if ($f['visibility'] === 'dept')     return $admin || in_array($f['dept_slug'], $userDeptSlugs);

        // Personal folders are scoped to their owner only — even for admins, so

        // one agent's "My Documents" section never shows another agent's files.

        if ($f['visibility'] === 'personal') return ($f['owner_email'] ?? '') === $email;
...
$rootFolders = vault_visible_root_folders($admin, $userDeptSlugs, $email);



// Group root folders: personal first, then public, then by dept.

$grouped = ['__personal' => [], '__public' => []];

foreach ($allDepts as $d) $grouped[$d['slug']] = [];

foreach ($rootFolders as $f) {

    if ($f['visibility'] === 'personal') {

        $key = '__personal';

    } elseif ($f['visibility'] === 'dept' && $f['dept_slug']) {
  ...
  <div class="content">
    ...
    <div class="vault-layout">
      ...
      <div class="vault-tree" id="vault-tree">
        ...
        $deptNames = ['__personal' => 'My Documents', '__public' => 'Public'];
        ...
        foreach ($grouped as $groupKey => $folders) {
            ...
            if (empty($folders) && $groupKey !== '__public' && $groupKey !== '__personal') continue;
...
function renderFolder(data, name) {
    ...
    const canWrite = admin || !!data.can_write;
    ...
    if (data.files && data.files.length) {
        ...
        for (const f of data.files) {
            ...
            if (canWrite) html += '<td><button class="btn-sm btn-danger" onclick="deleteFile(\'' + f.id + '\',\'' + f.name.replace(/'/g,"\\'") + '\')">Delete</button></td>';
    ...
    if ((!data.folders || !data.folders.length) && (!data.files || !data.files.length)) {
        ...
        html += '<div class="vault-empty">This folder is empty.' + (canWrite ? '<br><br><button class="btn-sm btn-green" onclick="showUpload()">⬆ Upload a file</button>' : '') + '</div>';
...
function resetVault() {
    ...
    document.getElementById('vault-main').innerHTML = '<div class="vault-empty"><div style="font-size:40px;margin-bottom:12px"></div><div style="font-size:15px;font-weight:600;color:#555;margin-bottom:6px">The Vault</div><div>Select a folder from the left to browse files.</div></div>';
    ...
    const uploadNext = (i) => {
        ...
        if (i >= total) {
            ...
            if (failed.length === 0) {
                ...
                vaultToast(`Upload failed — 0 of ${total} file(s) uploaded. Check your connection and try again.`);

            } else {

                vaultToast(`${total - failed.length} of ${total} uploaded — ${failed.length} failed: ${failed.join(', ')}`);
        ...
        .then(d => {
            ...
            if (!d.ok) { failed.push(file.name); vaultToast('Save failed: ' + file.name); }
