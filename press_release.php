...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_contact') {
        ...
        if ($category && $outlet) {
            ...
            $db->prepare("INSERT INTO press_contacts (category,outlet,beat,how,note,sort_ord) VALUES (?,?,?,?,?,?)")
               ->execute([$category,$outlet,$beat,$how,$note,$ord]);
        }
        ...
        if ($id && $category && $outlet) {
            $db->prepare("UPDATE press_contacts SET category=?,outlet=?,beat=?,how=?,note=? WHERE id=?")
               ->execute([$category,$outlet,$beat,$how,$note,$id]);
        }
...
<?php endif; ?>
            ...
            <form class="ct-edit-form" id="ct-edit-<?= $cid ?>" method="post" style="display:none">
              <input type="hidden" name="csrf" value="<?= h($csrf) ?>">
