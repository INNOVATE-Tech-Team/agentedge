<?php
// Public privacy policy page — no login required.
?><!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Privacy Policy — INNOVATE Real Estate</title>
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;color:#222;background:#fff;line-height:1.7}
  .wrap{max-width:760px;margin:0 auto;padding:60px 24px}
  h1{font-size:28px;font-weight:800;margin-bottom:6px}
  .updated{font-size:13px;color:#888;margin-bottom:40px}
  h2{font-size:17px;font-weight:700;margin:36px 0 10px;color:#000}
  p{margin-bottom:14px;font-size:15px;color:#333}
  ul{margin:0 0 14px 20px;font-size:15px;color:#333}
  li{margin-bottom:6px}
  a{color:#82C112;text-decoration:none}
  a:hover{text-decoration:underline}
  .contact-box{background:#f7f7f7;border-radius:8px;padding:20px 24px;margin-top:10px;font-size:15px}
  hr{border:none;border-top:1px solid #eee;margin:40px 0}
</style>
</head>
<body>
<div class="wrap">
  <h1>Privacy Policy</h1>
  <p class="updated">Last updated: July 13, 2026</p>

  <p>INNOVATE Real Estate ("INNOVATE," "we," "our," or "us") operates internal agent tools and platforms including AgentEdge, INNOVATE Advantage (bold360.vip), agent websites, and related services. This Privacy Policy explains how we collect, use, and protect information in connection with these platforms.</p>

  <h2>1. Who This Policy Covers</h2>
  <p>This policy applies to INNOVATE agents, staff, and brokers who use our internal platforms. It does not apply to the general public or to third-party websites we may link to.</p>

  <h2>2. Information We Collect</h2>
  <ul>
    <li><strong>Account information</strong> — name, email address, role, and Market Center affiliation, provided at account creation or imported from our brokerage records.</li>
    <li><strong>Authentication data</strong> — hashed passwords and session tokens stored securely in our database. We never store passwords in plain text.</li>
    <li><strong>DotLoop OAuth tokens</strong> — when you connect your DotLoop account through AgentEdge, we store your DotLoop OAuth access token and refresh token to retrieve your transaction data on your behalf. We store only what is necessary to maintain your connection.</li>
    <li><strong>Transaction and MLS data</strong> — transaction details (closing dates, prices, commission figures) retrieved from DotLoop and MLS feeds are displayed within our internal tools and are not shared externally.</li>
    <li><strong>Usage data</strong> — standard server logs including IP address, browser type, and pages accessed, used for security monitoring and debugging.</li>
    <li><strong>Communications</strong> — notes, tags, and messages created within INNOVATE Advantage (the CRM) in the course of managing agent relationships.</li>
  </ul>

  <h2>3. How We Use Your Information</h2>
  <ul>
    <li>To provide and operate our internal agent and broker tools</li>
    <li>To display your DotLoop transactions, documents, and deal details within AgentEdge</li>
    <li>To send system notifications such as @mention alerts and password reset emails</li>
    <li>To support onboarding, training tracking, and performance reporting within the brokerage</li>
    <li>To ensure platform security and prevent unauthorized access</li>
  </ul>
  <p>We do not sell your personal information. We do not share it with third parties except as described in Section 4.</p>

  <h2 id="sms">4. SMS / Text Message Notifications</h2>
  <p><strong>Program name:</strong> AgentEdge SMS Notifications, operated by INNOVATE Real Estate.</p>
  <p><strong>This page is the complete, verifiable record of this opt-in flow — no login required.</strong> AgentEdge is an internal agent portal, so the control described below lives behind an authenticated "My Profile" page and cannot itself be linked to directly. Below are two real screenshots taken directly from a live AgentEdge account's My Profile page, showing this exact control in both states, so the opt-in can be fully verified without needing a login.</p>
  <p><strong>How you opt in.</strong> An existing AgentEdge agent signs in to their account, goes to My Profile, and finds the Notification Preferences card shown below. There they can turn on "Text (SMS) notifications" and enter a US mobile number.</p>
  <p><strong>Before opting in</strong> — the toggle is unchecked by default:</p>
  <img src="/assets/sms-notif-unchecked.png" alt="AgentEdge My Profile Notification Preferences card, Text (SMS) notifications unchecked by default" style="max-width:420px;width:100%;border:1px solid #ddd;border-radius:8px;display:block">
  <p style="margin-top:20px"><strong>After opting in</strong> — checking the box reveals the phone number field and the full consent disclosure:</p>
  <img src="/assets/sms-notif-checked.png" alt="AgentEdge My Profile Notification Preferences card, Text (SMS) notifications checked, showing phone number field and consent disclosure text" style="max-width:420px;width:100%;border:1px solid #ddd;border-radius:8px;display:block">
  <p style="margin-top:14px"><strong>This is not required.</strong> The checkbox above is unchecked by default and lives entirely inside optional Notification Preferences — every other AgentEdge feature (login, transactions, University, tickets, etc.) works identically whether or not it's ever turned on. Enabling it is the only thing that constitutes consent; nothing else on AgentEdge depends on it.</p>
  <p><strong>Opt-in confirmation.</strong> The moment an agent enables the toggle above, they immediately receive this text: <em>"INNOVATE Real Estate: You're opted in for account &amp; company text notifications. Msg frequency varies (a few/month). Msg&amp;Data rates may apply. Reply STOP to cancel, HELP for help."</em> That confirms the number, restates the program, and repeats opt-out instructions before any other message is ever sent.</p>
  <p><strong>Who this applies to.</strong> Only INNOVATE agents and staff with an active AgentEdge login who have affirmatively enabled this toggle. This is not used for cold outreach or marketing to the public.</p>
  <p><strong>What you'll receive.</strong> Short alerts when a new company announcement is posted, e.g.: <em>"INNOVATE: Office closed Friday for training — see AgentEdge for details."</em> Roughly a few messages per month, tied to real announcement volume.</p>
  <p><strong>Opting out.</strong> Reply <strong>STOP</strong> to any message to unsubscribe immediately, or uncheck the toggle in My Profile at any time. Reply <strong>HELP</strong> for assistance.</p>
  <p><strong>Support.</strong> Reply <strong>HELP</strong> to any message, or contact <a href="mailto:info@innovateonline.com">info@innovateonline.com</a> with any questions about this program.</p>

  <h2>5. Third-Party Services</h2>
  <ul>
    <li><strong>DotLoop (Zillow Group)</strong> — when you connect your DotLoop account, you authorize us to access your transaction data through DotLoop's API on your behalf. DotLoop's own privacy policy governs their handling of your data.</li>
    <li><strong>SendGrid (Twilio)</strong> — we use SendGrid to send transactional emails (notifications, password resets). Your email address is transmitted to SendGrid for delivery purposes only.</li>
    <li><strong>Trestle / MLS</strong> — agent and listing data is retrieved from MLS data feeds under applicable data licensing agreements and is used solely for internal brokerage operations.</li>
    <li><strong>Amazon Web Services</strong> — our servers and databases are hosted on AWS infrastructure in the United States.</li>
  </ul>

  <h2>6. Data Security</h2>
  <p>We use industry-standard measures to protect your data, including encrypted HTTPS connections, hashed password storage, and access controls that limit data visibility to authorized users only. OAuth tokens are stored encrypted at rest.</p>

  <h2>7. Data Retention</h2>
  <p>We retain your account information for as long as you are affiliated with INNOVATE. When an agent or staff member's account is deactivated, personal data is retained for up to 12 months for auditing purposes, then deleted or anonymized. DotLoop OAuth tokens are deleted immediately upon disconnecting your account.</p>

  <h2>8. Your Rights</h2>
  <p>You may request access to, correction of, or deletion of your personal information at any time by contacting us using the information below. To disconnect your DotLoop account and remove stored tokens, you can do so directly within AgentEdge at any time.</p>

  <h2>9. Children's Privacy</h2>
  <p>Our platforms are intended solely for real estate professionals and are not directed at individuals under 18 years of age.</p>

  <h2>10. Changes to This Policy</h2>
  <p>We may update this policy from time to time. The "Last updated" date at the top of this page will reflect any changes. Continued use of our platforms after changes are posted constitutes acceptance of the updated policy.</p>

  <h2>11. Contact Us</h2>
  <p>If you have questions or requests regarding this Privacy Policy, please contact:</p>
  <div class="contact-box">
    <strong>INNOVATE Real Estate</strong><br>
    1309 Professional Drive<br>
    Myrtle Beach, SC 29577<br>
    <a href="mailto:info@innovateonline.com">info@innovateonline.com</a>
  </div>

  <hr>
  <p style="font-size:13px;color:#aaa;text-align:center;">&copy; 2026 INNOVATE Real Estate. All rights reserved.</p>
</div>
</body>
</html>
