...
<div class="modal-overlay" id="mls-activity-modal">
  <div class="modal">
    ...
    <div class="modal-body">
      ...
      <div class="form-section">
        <div class="form-section-title">Set a Reminder</div>
        <div style="display:flex;gap:8px;align-items:flex-end;flex-wrap:wrap;margin-bottom:8px">
          <div class="field" style="min-width:160px;flex:1">
            <label>Date</label>
            <input type="date" id="activity-reminder-date" style="padding:7px 10px;border:1px solid #ddd;border-radius:6px;font-size:13px;width:100%;box-sizing:border-box">
          </div>
          <div class="field" style="min-width:220px;flex:3">
            <label>Note (optional)</label>
            <input type="text" id="activity-reminder-note" placeholder="e.g. Follow up on approval status" style="padding:7px 10px;border:1px solid #ddd;border-radius:6px;font-size:13px;width:100%;box-sizing:border-box">
          </div>
          <button class="btn-ghost" id="activity-reminder-add-btn" onclick="addActivityReminder()" style="white-space:nowrap;padding:7px 14px">Set Reminder</button>
        </div>
        <div id="activity-reminders-list" style="display:flex;flex-direction:column;gap:6px"></div>
      </div>
...
function openActivityModal(id){
  ...
  document.getElementById('activity-reminder-date').value = '';
  document.getElementById('activity-reminder-note').value = '';
  document.getElementById('mls-activity-modal').classList.add('open');
  loadActivityNotes();
  loadActivityAgreements();
  loadActivityReminders();
}
...
// ── Reminders ─────────────────────────────────────────────────────────────────
function loadActivityReminders(){
  const list = document.getElementById('activity-reminders-list');
  list.innerHTML = '';
  fetch('api/mls_reminders.php?mls_id='+activityMlsId, {credentials:'same-origin'})
    .then(r=>r.json()).then(d=>{
      if(!d.ok || !d.reminders.length){ list.innerHTML=''; return; }
      const today = new Date().toISOString().slice(0,10);
      list.innerHTML = d.reminders.map(r=>{
        const overdue = !r.dismissed_at && r.remind_at < today;
        const dismissed = !!r.dismissed_at;
        const dateLabel = r.remind_at.slice(0,10).split('-').map((v,i)=>i===0?v:[v]).reduce((a,b,i)=>i===1?b+'/'+a:a+'/'+b);
        return `<div style="display:flex;align-items:flex-start;gap:10px;padding:8px 12px;border-radius:6px;border:1px solid ${dismissed?'#eee':overdue?'#fde68a':'#bfdbfe'};background:${dismissed?'#fafafa':overdue?'#fffbeb':'#eff6ff'};opacity:${dismissed?'.6':'1'}">
          <div style="flex:1;min-width:0">
            <div style="font-weight:700;font-size:13px;color:${overdue&&!dismissed?'#b45309':'#333'}">${esc(fmtReminderDate(r.remind_at))}${overdue&&!dismissed?' <span style="font-size:10px;font-weight:700;color:#b45309;margin-left:4px">OVERDUE</span>':''}${dismissed?' <span style="font-size:10px;color:#888">dismissed</span>':''}</div>
            ${r.note?`<div style="font-size:13px;color:#555;margin-top:2px">${esc(r.note)}</div>`:''}
            <div style="font-size:11px;color:#999;margin-top:2px">Set by ${esc(r.created_by)}</div>
          </div>
          <div style="display:flex;flex-direction:column;gap:3px;align-items:flex-end">
            ${!dismissed?`<button class="btn-sm" onclick="dismissActivityReminder(${r.id})">Dismiss</button>`:''}
            <button class="btn-sm btn-danger" onclick="deleteActivityReminder(${r.id})">Delete</button>
          </div>
        </div>`;
      }).join('');
...
function fmtReminderDate(s){
  if(!s) return '—';
  const p=s.slice(0,10).split('-');
  return p[1]+'/'+p[2]+'/'+p[0];
}

function addActivityReminder(){
  const remindAt = document.getElementById('activity-reminder-date').value;
  if(!remindAt){ alert('Pick a reminder date first.'); return; }
  const note = document.getElementById('activity-reminder-note').value.trim();
  const btn = document.getElementById('activity-reminder-add-btn');
  btn.disabled=true; btn.textContent='Saving…';
  fetch('api/mls_reminders.php', {method:'POST', credentials:'same-origin', headers:{'Content-Type':'application/json'},
    body: JSON.stringify({action:'add', mls_id:activityMlsId, remind_at:remindAt, note})})
    .then(r=>r.json()).then(d=>{
      btn.disabled=false; btn.textContent='Set Reminder';
      if(d.ok){ document.getElementById('activity-reminder-date').value=''; document.getElementById('activity-reminder-note').value=''; loadActivityReminders(); }
      else alert(d.error||'Failed to set reminder.');
    }).catch(()=>{ btn.disabled=false; btn.textContent='Set Reminder'; alert('Request failed.'); });
}

function dismissActivityReminder(id){
  fetch('api/mls_reminders.php', {method:'POST', credentials:'same-origin', headers:{'Content-Type':'application/json'},
    body: JSON.stringify({action:'dismiss', id})})
    .then(r=>r.json()).then(d=>{ if(d.ok) loadActivityReminders(); else alert(d.error||'Failed.'); })
    .catch(()=>alert('Request failed.'));
}

function deleteActivityReminder(id){
